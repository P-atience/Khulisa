...
function calculateScores($data) {
    ...
    if ($data["credit_score"] >= 650) $credit = 100;
    elseif ($data["credit_score"] >= 550) $credit = 60;
    else $credit = 30;
...
function createGaps($conn, $user_id, $assessment_id, $data) {
    ...
    $stmt = $conn->prepare("
        INSERT INTO gaps 
        (user_id, assessment_id, category, description, severity)
        VALUES (?, ?, ?, ?, ?)
    ");

    if (!$stmt) {
        die("Gap prepare failed: " . $conn->error);
    }
    ...
    foreach ($gaps as $gap) {
        ...
        if (!$stmt->execute()) {
            die("Gap insert failed: " . $stmt->error);
        }
    }
}
